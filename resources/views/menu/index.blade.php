@php
    ...
    $__stackTail = "'Outfit', 'Noto Sans Arabic', ui-sans-serif, system-ui, sans-serif";
    $__chosenFont = $settings->primaryFontFamilyForMenuLocale($lang);
    $__fontSansCss = $__chosenFont !== null
        ? json_encode($__chosenFont, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS).', '.$__stackTail
        : $__stackTail;
@endphp
<!DOCTYPE html>
<html lang="{{ \App\Support\MenuLocale::htmlLang($lang) }}" dir="{{ \App\Support\MenuLocale::isRtl($lang) ? 'rtl' : 'ltr' }}" data-flm-lang="{{ $lang }}">
<head>
    <meta charset="utf-8">
    ...
    <link href="{{ $settings->googleFontsStylesheetHref() }}" rel="stylesheet">
    <script>
        (function () {
            try {
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* UA styles reset: buttons ignore inherited font-family unless set explicitly */
        button {
            font-family: var(--font-sans);
        }
...
<body>
    ...
    @php
        ...
        $hasHeroInner = $s->hasAnySiteName() || $s->logo_url || $s->phone || $s->hasAnyAddress()
            ...
            || $s->social_tiktok_url || $s->social_youtube_url || $s->social_snapchat_url;
        ...
        <header class="menu-cover">
            ...
            @if ($hasHeroInner)
                <div class="menu-cover-inner">
                    ...
                    @endif
                    @if ($s->social_facebook_url || $s->social_instagram_url || $s->social_twitter_url || $s->social_tiktok_url || $s->social_youtube_url || $s->social_snapchat_url)
                        <div class="cover-socials">
                            ...
                            @endif
                            @if ($s->social_snapchat_url)
                                <a href="{{ $s->social_snapchat_url }}" target="_blank" rel="noopener noreferrer" aria-label="Snapchat">
                                    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.174-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.127.889 2.722.099.117.127.219.091.344-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.402.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.92-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.354-.629-2.758-1.378l-.749 2.848c-.269 1.045-1.004 2.352-1.498 3.146 1.123.345 2.306.535 3.55.535 6.624 0 11.99-5.367 11.99-11.988C24.007 5.367 18.641.001 12.017.001z"/></svg>
                                </a>
                            @endif
